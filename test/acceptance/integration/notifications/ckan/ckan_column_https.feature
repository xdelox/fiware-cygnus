...
Feature: Stored in ckan new notifications per column from context broker
  ...
  Examples:
    ...
    | cygnus_col_023 | myserv6      | 1                  | pressure       | celsius        | 46.3              | True           | json    |
    | cygnus_col_023 | myserv6      | 1                  | pressure       | celsius        | 46.3              | True           | xml     |
  ...
  Scenario Outline: store in ckan new notifications with different resource behavior and with the same service_path
    ...
    And verify if cygnus is installed correctly
    And verify if ckan is installed correctly
    And service "org_col_multi_resource_001", service path "/test_02", entity type "<entity_type>", entity id "<entity_id>", with attribute number "1", attribute name "temperature" and attribute type "celsius"
    And create a new organization with a dataset and a new resource with attribute data type "json" and metadata data type "json"
    When receives a notification with attributes value "random", metadata value "False" and content "<content>"
    Then receive an "OK" http code
    And Verify that the attribute value is stored in ckan
  ...
  Scenario Outline: store in ckan new notifications with different resources behavior and different service path
    ...
    And verify if cygnus is installed correctly
    And verify if ckan is installed correctly
    And service "resource_multi_02110", service path "<service_path>", entity type "<entity_type>", entity id "<entity_id>", with attribute number "1", attribute name "temperature" and attribute type "celsius"
    And create a new organization with a dataset and a new resource with attribute data type "json" and metadata data type "json"
    When receives a notification with attributes value "random", metadata value "False" and content "<content>"
    Then receive an "OK" http code
    And Verify that the attribute value is stored in ckan
  ...
  Scenario Outline:  store in ckan new notifications with different quantities of attributes
    ...
    And verify if cygnus is installed correctly
    And verify if ckan is installed correctly
    And service "<organization>", service path "/test", entity type "room", entity id "room2", with attribute number "<attribute_number>", attribute name "temperature" and attribute type "celsius"
    And create a new organization with a dataset and a new resource with attribute data type "json" and metadata data type "json"
    When receives a notification with attributes value "random", metadata value "False" and content "<content>"
    Then receive an "OK" http code
    And Verify that the attribute value is stored in ckan
  ...
  Scenario Outline: try to store new notification in ckan without value or metadata fields
    ...
    And verify if cygnus is installed correctly
    And verify if ckan is installed correctly
    And service "without_fields_0311", service path "/test", entity type "room", entity id "room2", with attribute number "1", attribute name "temperature" and attribute type "celsius"
